...
var roundFunc = function(dt) {
	...
	{
		scene.timeOut();
		scene.layers[scene.nextLevelID-1].wrongSFX.play();
...
cu.TimerPanel = function() {
	...
	this.gameTime = 0;
	this.gameTimeLabel = new lime.Label().setSize(100,100).setFontSize(20).setPosition(-20,-25);
	this.roundTime = 0;
	this.roundTimeLabel = new lime.Label().setSize(100,100).setFontSize(50).setFontFamily('impact').setPosition(-50,-10);
...
cu.TimerPanel.prototype.endGameDisplay = function(time)
{
	...
	this.gameTimeLabel.runAction(new lime.animation.Spawn(
		new lime.animation.MoveTo(this.gameTimeLabel.getPosition().x - 45,120),
